...
## Общее описание
Проект содержит автотесты пользовательского интерфейса для страницы Form Fields сайта Practice Automation. Набор сценариев покрывает ключевые проверки формы: корректное заполнение всех обязательных полей, позитивный сценарий только с именем и негативный кейс без имени. Тесты запускаются в Chrome, снимается скриншоты при падениях кейсов, а результат собирается в [Allure отчет](https://zen2281488.github.io/Silicium_3_0).
